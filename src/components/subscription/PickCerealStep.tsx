  ...
  return (
    <div className="mx-auto px-2">
      ...
      >
        ...
        {/* Cereal grid — 2×2 on mobile, 1×4 on desktop */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-[6.24px] p-[6.24px]">
          {CEREALS.map((cereal) => {
            ...
            return (
              ...
              >
                {/* Radio indicator */}
                <div className="absolute top-[8px] right-[8px] z-10 w-[20.557px] h-[20.557px] lg:w-[28px] lg:h-[28px] rounded-full border-[1.5px] border-black/70 bg-white/60 flex items-center justify-center">
                  {isSelected && (
                    <div className="w-[11.42px] h-[11.42px] lg:w-[15px] lg:h-[15px] rounded-full bg-ink" />
                  )}
                </div>

                {/* Bowl image */}
                <div className="w-[126px] h-[92px] lg:w-[180px] lg:h-[130px] relative mb-[8px] lg:mb-[12px] shrink-0">
                  <Image src={cereal.image} alt={cereal.name} fill className="object-contain" />
                </div>

                {/* Name */}
                <p className="text-[22px] phone:text-[24px] lg:text-[28px] font-semibold text-ink leading-none tracking-[-0.4274px] text-center mb-[6px]">
                  {cereal.name}
                </p>
        ...
        {plan === "monthly" && (
          <div className="px-4 pb-4 pt-2">
            <p className="text-[23px] phone:text-[25px] lg:text-[28px] font-semibold text-ink mb-2">
              2. How many boxes of {selectedCerealName}?
            </p>
            <div className="flex items-center gap-4">
              <div className="flex items-stretch h-[44px] lg:h-[52px] rounded-[6px] border border-[#7D6A5F] overflow-hidden shrink-0">
                <button
                  className="w-[44px] lg:w-[52px] flex items-center justify-center text-[22px] lg:text-[26px] font-semibold text-ink bg-[rgba(223,209,203,0.6)] hover:bg-[rgba(223,209,203,0.8)]"
                  onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                >
                  −
                </button>
                <span className="w-[44px] lg:w-[52px] flex items-center justify-center text-[20px] lg:text-[24px] font-semibold text-white bg-[#3A4041] border-l border-r border-[#7D6A5F]">
                  {quantity}
                </span>
                <button
                  className="w-[44px] lg:w-[52px] flex items-center justify-center text-[22px] lg:text-[26px] font-semibold text-ink bg-[rgba(223,209,203,0.6)] hover:bg-[rgba(223,209,203,0.8)]"
              ...
              <span className="text-[18px] phone:text-[20px] lg:text-[22px] font-normal text-ink leading-snug">
      ...
      {/* Checkout button */}
      <button
        className="relative w-full lg:w-[500px] lg:mx-auto lg:block h-[50px] lg:h-[64px] overflow-hidden rounded-[7.53px] border-[2.259px] border-border-outline shadow-[0px_5.271px_3.012px_0px_black,0px_6.024px_9.036px_0px_rgba(0,0,0,0.25)] active:shadow-none active:translate-y-[4px] transition-all"
        onClick={onCheckout}
      >
        ...
        <span className="relative z-10 text-[29.171px] lg:text-[36px] font-semibold text-footer whitespace-nowrap">
          Checkout
        </span>
      </button>

      {/* Ships in 2-3 days — desktop */}
      <p className="hidden lg:flex items-center justify-center gap-2 text-[22px] font-normal text-ink mt-3">
        <span>🚚</span> Ships within 2-3 days.
      </p>
